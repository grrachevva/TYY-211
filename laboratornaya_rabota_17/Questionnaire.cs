    ...
    public partial class FormQuestionnaire : Form
    {
        ...
        private void btnGenerateWord_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                AddParagraph(doc, $"Семейное положение: {txtSemPolozh.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Состояние здоровья: {txtHealth.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Ограничения по здоровью: {txtOgrPoZd.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Наличие личного транспорта: {txtLichTr.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Должность: {txtPosition.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Зарплатные ожидания: {txtSalaryExpectation.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Когда приступить к работе: {dateTimePristKRab.Value.ToShortDateString()}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
                AddParagraph(doc, $"Образование: {tbEducation.Text}", 12, false, Word.WdParagraphAlignment.wdAlignParagraphLeft);
